...
def main():
    ...
    try:
        result = pipeline.execute(cities=["Barcelona"], days_ahead=180)
            ...
            for idx, event in enumerate(result.events[:5], 1):
                ...
                if event.taxonomy_dimensions:
                    ...
                    for dim in event.taxonomy_dimensions[:2]:
                        print(
                            f"        - {dim.primary_category} ({dim.subcategory}): {dim.confidence:.2f}"
                        )
                print()

            # Show summary
            print(
                f"   📊 Total unique cities: {len(set(e.location.city for e in result.events))}"
            )
            avg_quality = sum(e.data_quality_score for e in result.events) / len(
                result.events
            )
