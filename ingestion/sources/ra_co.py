...
from normalization.event_schema import (
    EventSchema, EventType, LocationInfo, Coordinates,
    PriceInfo, TicketInfo, OrganizerInfo, SourceInfo,
    MediaAsset, EngagementMetrics, TaxonomyDimension,
    PrimaryCategory, Subcategory, EventFormat
)
...
class RaCoEventPipeline(BasePipeline):
    ...
    def fetch_raw_data(self, cities: Optional[List[str]] = None, 
                      days_ahead: int = 90, **kwargs) -> List[Dict[str, Any]]:
        """
    ...
    def parse_raw_event(self, raw_event: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            parsed = {
                ...
                "area": venue.get("area", {}).get("name") if venue.get("area") else None,
                "country": venue.get("country", {}).get("name") if venue.get("country") else None,
                "country_code": venue.get("country", {}).get("urlCode") if venue.get("country") else "GB",
    ...
    def normalize_to_schema(self, parsed_event: Dict[str, Any],
        ...
        cost_str = parsed_event.get("cost", "").strip() if parsed_event.get("cost") else ""
        ...
        price = PriceInfo(
            ...
            maximum_price=None,
            early_bird_price=self._parse_price(cost_str.split('-')[0]) if not is_free and '-' in cost_str else None,
            standard_price=self._parse_price(cost_str.split('-')[-1]) if not is_free else None,
            vip_price=None,
