...
class BasePipeline(ABC):
    ...
    def execute(self, **kwargs) -> PipelineExecutionResult:
        ...
        try:
            ...
            self.logger.info(f"Pipeline execution completed: {result.successful_events}/{result.total_events_processed} successful")
            return result
    ...
    def _process_events_batch(self, raw_events: List[Dict[str, Any]]) -> List[EventSchema]:
        ...
        for idx, raw_event in enumerate(raw_events):
            try:
                ...
                if not is_valid:
                    self.logger.warning(f"Event validation warnings: {validation_messages}")
                ...
                self.logger.error(f"Failed to process event {idx+1}: {str(e)}", exc_info=True)
    ...
    def _calculate_quality_score(self, event: EventSchema) -> float:
        ...
        if event.taxonomy_dimensions:
            avg_confidence = sum(dim.confidence for dim in event.taxonomy_dimensions) / len(event.taxonomy_dimensions)
