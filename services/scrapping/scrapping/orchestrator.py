                    ...
                    issues.append(
                        {
                            "level": "warning",
                            "code": "broad_regex",
                            "msg": f"{sid}: link_extract pattern '{pattern}' is too broad",
                        }
                    )
                ...
                issues.append(
                    {
                        ...
                        "msg": f"{sid}: engine type {et} requires playwright which is not installed",
                    }
                )
            ...
            issues.append(
                {
                    "level": "warning",
                    "code": "ssl_disabled",
                    "msg": f"{sid}: verify_ssl is set to false; this is discouraged",
                }
            )
            ...
            try:
                ...
                with sync_playwright() as pw:
                    ...
                    try:
                        ...
                        if (
                            "error while loading shared libraries" in err_msg
                            or "cannot open shared object file" in err_msg
                        ):
                        ...
                        return (
                            False,
                            f"Browser binaries missing or failed to launch: {err_msg}",
                            hint,
                            None,
                        )
            except Exception as e:
...
class Orchestrator:
    ...
    def run(self, cfg: dict[str, Any]) -> dict[str, Any]:
        ...
        for s in sources:
            ...
            try:
                    ...
                    for trace_entry in lp.fetch.engine_trace:
                        if "actions" in trace_entry and trace_entry["actions"]:
                            emit_event(
                                ...
                                {"url": lp.url, "actions": trace_entry["actions"]},
                            )
                    ...
                    for trace_entry in dp.fetch.engine_trace:
                        if "actions" in trace_entry and trace_entry["actions"]:
                            emit_event(
                                slog,
                                "source.browser.actions",
                                {"url": dp.url, "actions": trace_entry["actions"]},
                            )
                ...
                report.metrics.inc(
                    "pages_fetched",
                    artifacts.stats.pages_succeeded,
                    labels={"source_id": source_id},
                )
                report.metrics.inc(
                    "items_saved",
                    artifacts.stats.items_saved,
                    labels={"source_id": source_id},
                )
    ...
    def _make_run_meta(
        self, run_id: str, report: RunReportBuilder, cfg: dict[str, Any]
    ) -> dict[str, Any]:
        meta = {
            "run_id": run_id,
    ...
    def _write_artifacts(
        ...
        links_path = write_links(
            layout, run_id, source_id, artifacts.extracted_links, options=writer_opts
        )
