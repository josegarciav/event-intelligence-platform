...
from scrapping.actions.browser_actions import (
    ActionResult,
)
...
def test_ensure_started_missing_browsers(mock_pw):
    ...
    mock_launcher = mock_pw_instance.chromium
...
async def test_browser_engine_sync_call_in_async_loop():
    ...
    with patch("playwright.sync_api.sync_playwright"):
        ...
        with patch.object(
            BrowserEngine, "_get_rendered_sync", wraps=engine._get_rendered_sync
        ) as mock_sync:
            # Mock the internal ensure_started to avoid real PW launch but keep logic
            with patch.object(BrowserEngine, "_ensure_started"):
                engine.get_rendered("https://example.com")
