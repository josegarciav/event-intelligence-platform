...
from scrapping.runtime.results import EngineError, FetchResult
...
def test_fetch_result_retryable():
    ...
    res = FetchResult(
        final_url="http://a",
        error=EngineError(type="T", message="M", is_retryable=True),
    )
    assert res.ok is False
    assert res.is_retryable is True
...
def test_block_detection():
    ...
    assert "captcha_present" in [s.value for s in signals] or "likely_blocked" in [
