...
class PromptLoader:
    ...
    def get_prompt(self, group: str, feature: str, variables: Dict[str, Any]) -> Dict[str, str]:
        """
        ...
        if "taxonomy_context" not in variables:
            ...
            if sub_id:
                variables["taxonomy_context"] = self.taxonomy.get_subcategory_context_for_prompt(sub_id)
            else:
                variables["taxonomy_context"] = self.taxonomy.get_all_categories_summary()

        # 2. Inject standard allowed attribute values (energy_level, etc.)
        variables["attribute_options_string"] = self.taxonomy.get_attribute_options_string()
    ...
    def get_all_group_prompts(self, variables: Dict[str, Any]) -> Dict[str, Dict[str, str]]:
        """
